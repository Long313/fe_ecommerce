        ...
        <div className={`bg-[#fff] z-40 h-[200px] pt-[40px] px-[var(--padding-screen)] flex flex-col transition-transform ease-in-out duration-500 ${show ? "translate-y-0" : "-translate-y-full"}`}>
            <div className="flex flex-row justify-between items-start">
                ...
                <div className="flex flex-1 flex-col mx-[40px]">
                    ...
                    <div className="flex flex-wrap overflow-y-hidden gap-y-4 max-h-[60px]">
                        {listPropose.length > 0 && listPropose.map((name, index) => {
                            return (
                                <ProposeTag onClear={handleClear} key={index} name={name} onChoose={(value) => handleSearchByKey(value)} />
                            )
                        })}
                    </div>
                </div>
